...
texture AL_ScnMapRT: OFFSCREENRENDERTARGET <
    string Description = "ScanMapRenderTarget for AutoLuminous.fx";
    float2 ViewPortRatio = {1.0,1.0};
    float4 ClearColor = { 0, 0, 0, 1 };
    float ClearDepth = 1.0;
    bool AntiAlias = true;
...
sampler AL_ScnMapView = sampler_state {
    texture = <AL_ScnMapRT>;
    MinFilter = Linear;
    MagFilter = Linear;
    MipFilter = None;
...
DoubleColor PS_DrawHighLight( float2 Tex: TEXCOORD0 ) {
    ...
    //元スクリーンの高輝度成分の抽出
    OrgColor = tex2Dlod(AL_ScnMapView, float4(Tex, 0, 0));
...
float4 PS_AL_Mix( float2 Tex: TEXCOORD0 , uniform bool FullOut) : COLOR {
    ...
    float4 basecolor = tex2D(AL_ScnMapView, Tex);
